...
public:
    ...
    inline size_t GetNumberOfNormals() const { return normals.Size(); }
    inline size_t GetNumberOfColors3() const { return colors3.Size(); }
    inline size_t GetNumberOfColors4() const { return colors4.Size(); }
    inline size_t GetNumberOfUVs() const { return uvs.Size(); }
    inline size_t GetNumberOfIndices() const { return indices.Size(); }

    inline Eigen::Vector3f GetVertex(unsigned int index) const { return vertices.GetCpuData()[index]; }
    inline Eigen::Vector3f GetNormal(unsigned int index) const { return normals.GetCpuData()[index]; }
    inline Eigen::Vector3f GetColor3(unsigned int index) const { return colors3.GetCpuData()[index]; }
    inline Eigen::Vector4f GetColor4(unsigned int index) const { return colors4.GetCpuData()[index]; }
	...
	inline void SetColor3(unsigned int index, const Eigen::Vector3f& c) { colors3.SetData(index, c); }
	inline void SetColor4(unsigned int index, const Eigen::Vector4f& c) { colors4.SetData(index, c); }
	inline void SetUV(unsigned int index, const Eigen::Vector2f& uv) { uvs.SetData(index, uv); }
	inline void SetIndex(unsigned int index, unsigned int i) { indices.SetData(index, i); }

	inline void SetVertices(const std::vector<Eigen::Vector3f>& list) { vertices.SetDatas(list); }
	inline void SetNormals(const std::vector<Eigen::Vector3f>& list) { normals.SetDatas(list); }
	inline void SetColors3(const std::vector<Eigen::Vector3f>& list) { colors3.SetDatas(list); }
	inline void SetColors4(const std::vector<Eigen::Vector4f>& list) { colors4.SetDatas(list); }
	inline void SetUVs(const std::vector<Eigen::Vector2f>& list) { uvs.SetDatas(list); }
	inline void SetIndices(const std::vector<unsigned int>& list) { indices.SetDatas(list); }

	inline void ClearVertices() { vertices.Clear(); }
	inline void ClearNormals() { normals.Clear(); }
	inline void ClearColors3() { colors3.Clear(); }
	inline void ClearColors4() { colors4.Clear(); }
	inline void ClearUVs() { uvs.Clear(); }
	inline void ClearIndices() { indices.Clear(); }

    // Batch Data Access
    inline void AddVertices(const std::vector<Eigen::Vector3f>& list) { vertices.AddDatas(list); }
    inline void AddNormals(const std::vector<Eigen::Vector3f>& list) { normals.AddDatas(list); }
    inline void AddColors3(const std::vector<Eigen::Vector3f>& list) { colors3.AddDatas(list); }
    inline void AddColors4(const std::vector<Eigen::Vector4f>& list) { colors4.AddDatas(list); }
    inline void AddUVs(const std::vector<Eigen::Vector2f>& list) { uvs.AddDatas(list); }
    ...
    // Settings
	inline GeometryMode GetGeometryMode() const { return geometryMode; }
    inline void SetGeometryMode(GeometryMode mode) { geometryMode = mode; }
    ...
    inline void SetUseAlpha(bool use) { useAlpha = use; }
    inline bool IsUsingAlpha() const { return useAlpha; }

	inline const AttributeBuffer<Eigen::Vector3f>& GetVertexBuffer() const { return vertices; }
	inline const AttributeBuffer<Eigen::Vector3f>& GetNormalBuffer() const { return normals; }
	inline const AttributeBuffer<Eigen::Vector3f>& GetColor3Buffer() const { return colors3; }
