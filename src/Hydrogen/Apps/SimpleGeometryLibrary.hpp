...
namespace SGL
{
	...
	public:
		...
		// Planar inside test for a point on the cut plane z=cut_z: cast a +x ray and
		// count this mesh's cross-section edges at that plane; odd = inside. Robust
		// for points lying ON the cut plane, where nearest-face flips sign because
		// the closest face is a near-vertical wall. Use only for cut-plane-parallel
		// faces; use IsInsideByNearestFace for everything else.
		bool IsInsidePlanarRay(const Eigen::Vector3f& p, float cut_z) const
		{
			int crossings = 0;
			float cy = p.y();
			for (auto f = faces_begin(); f != faces_end(); ++f)
			{
				if (status(*f).deleted()) continue;
				Eigen::Vector3f v0, v1, v2;
				GetFaceVertices(*f, v0, v1, v2);
				float zmn = std::min({ v0.z(), v1.z(), v2.z() });
				float zmx = std::max({ v0.z(), v1.z(), v2.z() });
				if (zmn > cut_z || zmx < cut_z) continue;   // face must straddle the plane
				...
				for (int e = 0; e < 3; ++e)
				{
					...
					{
						float t = za / (za - zb);
						Eigen::Vector3f q = a + t * (b - a);
						hits.push_back(Eigen::Vector2f(q.x(), q.y()));
					}
				}
		...
		// Group cut faces that lie on the same plane and are connected, so they can
	// be triangulated together in ONE CDT. Splitting each coplanar face on its
	// own makes adjacent faces subdivide their shared edge differently, tearing
	// the seam; merging coplanar neighbors into one CDT removes that shared edge
	// from the problem entirely. Returns groups of face indices.
		std::vector<std::vector<int>> GroupCoplanarCutFaces(
			const std::set<int>& cut_faces, float normal_eps, float plane_eps) const
		{
			// plane signature per face: (normal, signed distance from origin)
			struct Plane { Eigen::Vector3f n; float d; };
			std::map<int, Plane> planes;
			for (int fi : cut_faces)
			{
				...
				if (n.squaredNorm() < 1e-20f) continue;
				n.normalize();
				planes[fi] = { n, n.dot(v0) };
			}

			// union-find over cut faces that are (a) coplanar and (b) edge-adjacent
			std::map<int, int> parent;
			for (int fi : cut_faces) parent[fi] = fi;
			...
			auto coplanar = [&](int a, int b) -> bool
				{
					auto ia = planes.find(a), ib = planes.find(b);
					if (ia == planes.end() || ib == planes.end()) return false;
					if (ia->second.n.dot(ib->second.n) < 1.0f - normal_eps) return false;
					return std::abs(ia->second.d - ib->second.d) < plane_eps;
				};

			// adjacency: faces sharing a mesh edge
			for (int fi : cut_faces)
			{
				OpenMesh::FaceHandle fh = face_handle(fi);
				if (!fh.is_valid() || status(fh).deleted()) continue;
				for (auto fh_it = cfh_iter(fh); fh_it.is_valid(); ++fh_it)
				{
					...
					int nj = nf.idx();
					if (cut_faces.count(nj) && coplanar(fi, nj))
						unite(fi, nj);
				}
			}

			std::map<int, std::vector<int>> buckets;
			for (int fi : cut_faces) buckets[find(fi)].push_back(fi);

			std::vector<std::vector<int>> groups;
			for (auto& kv : buckets) groups.push_back(kv.second);
			return groups;
		}
		...
		// Step 3-3 input check: do this face's constraint segments link into closed
		// rings? CDT preserves an outline only when fed a connected ring. Returns
		// the count of constraint vertices whose degree != 2 (0 means closed rings).
		// segs: list of canonical-id segment pairs for ONE face.
		int CountOpenRingVerts(const std::vector<std::pair<int, int>>& segs) const
		{
			std::map<int, int> deg;
		...
		// Step 3-3 output check: after splitting, is the cut curve a clean ring?
	// Each cut point (a pool point) must have exactly two incident cut edges.
	// Returns degree!=2 count via out_bad and isolated count via out_isolated;
	// out_total is the number of cut vertices found. 0 bad + 0 isolated = ring.
		void CheckCutRing(CanonicalPool& pool, float match_eps,
			int& out_total, int& out_deg2, int& out_bad, int& out_isolated) const
		{
			float match_eps_sq = match_eps * match_eps;

			std::vector<int> vtx_cut_id(n_vertices(), -1);
			for (auto v = vertices_begin(); v != vertices_end(); ++v)
			{
				Eigen::Vector3f p(point(*v).data());
				if (pool.Contains(p, match_eps))
				{
					for (int cid = 0; cid < (int)pool.Size(); ++cid)
						if ((pool.Point(cid) - p).squaredNorm() < match_eps_sq)
						{
							vtx_cut_id[v->idx()] = cid; break;
						}
				}
			}

			std::map<int, int> cut_degree;
			for (auto e = edges_begin(); e != edges_end(); ++e)
			{
				auto h = halfedge_handle(*e, 0);
				int va = from_vertex_handle(h).idx();
				int vb = to_vertex_handle(h).idx();
				if (vtx_cut_id[va] >= 0 && vtx_cut_id[vb] >= 0)
				{
					cut_degree[va]++;
					cut_degree[vb]++;
				}
			...
			out_total = (int)cut_degree.size();
			out_deg2 = 0; out_bad = 0; out_isolated = 0;
			for (const auto& kv : cut_degree)
			{
				if (kv.second == 2) out_deg2++;
				else out_bad++;
			}
			for (int vi = 0; vi < (int)vtx_cut_id.size(); ++vi)
				if (vtx_cut_id[vi] >= 0 && cut_degree.find(vi) == cut_degree.end())
					out_isolated++;
		}

		// Link a face's constraint segments (canonical-id pairs) into ordered
	// chains, then close near-open chains by joining endpoints that coincide
	// in position within eps. Returns each ring as an ordered list of canonical
	// ids (first id repeated at the end is NOT included; the ring is implicit).
	// This turns the loose intersection segments into the closed outline loops
	// that CDT's eraseOuterTrianglesAndHoles needs to preserve a ring.
		std::vector<std::vector<int>> LinkConstraintRings(
			const std::vector<std::pair<int, int>>& segs,
			CanonicalPool& pool, float close_eps) const
		{
			// adjacency by canonical id
			...
			auto mark = [&](int a, int b) { used.insert({ std::min(a,b), std::max(a,b) }); };
			auto is_used = [&](int a, int b) { return used.count({ std::min(a,b), std::max(a,b) }) > 0; };

			std::vector<std::vector<int>> rings;

			// walk chains: start from any unused edge, follow until we return or stall
			for (const auto& s : segs)
			{
				if (s.first == s.second || is_used(s.first, s.second)) continue;

				std::vector<int> chain;
				chain.push_back(s.first);
				int prev = s.first, cur = s.second;
				mark(prev, cur);
				chain.push_back(cur);
				...
				while (true)
				{
					...
					if (it != adj.end())
						for (int cand : it->second)
							if (cand != prev && !is_used(cur, cand)) { next = cand; break; }
					if (next < 0) break;
					mark(cur, next);
					chain.push_back(next);
					prev = cur; cur = next;
					if (cur == chain.front()) break;   // closed
				}
				rings.push_back(chain);
			}

			// close any open chains whose two endpoints coincide in position
			float eps_sq = close_eps * close_eps;
			for (auto& r : rings)
			{
				if (r.size() < 3) continue;
				if (r.front() == r.back()) { r.pop_back(); continue; }
				const Eigen::Vector3f& pa = pool.Point(r.front());
				const Eigen::Vector3f& pb = pool.Point(r.back());
				if ((pa - pb).squaredNorm() < eps_sq)
				{
					// endpoints coincide: already a closed ring, drop nothing
				}
				// else leave as open chain; CDT will still use it as a constraint
			}

			return rings;
		}

		// Split using canonical ids shared with the other mesh. Coplanar connected
	// cut faces are grouped and triangulated in a single CDT (see
	// SplitGroupShared) so shared interior edges (e.g. a quad diagonal) cannot
	// be subdivided inconsistently. Per-edge subdivisions on the group's outer
	// boundary are shared so neighbors split identically.
			...
			CanonicalPool& pool)
		{
			...
			const float close_eps = diag * 1e-4f;

			// shared per-edge subdivision points
			...
			// group coplanar connected cut faces so each group is one CDT
			auto groups = GroupCoplanarCutFaces(cut_faces, 1e-4f, diag * 1e-4f);
			...
			for (const auto& g : groups)
				SplitGroupShared(g, facePointIds, faceConstraints, edge_points,
					pool, merge_eps_sq, close_eps, local_id_counter,
					all_new_points, all_new_ids, all_new_indices);
			...
			for (auto f_it = faces_begin(); f_it != faces_end(); ++f_it)
			{
				...
				for (auto fv_it = cfv_iter(*f_it); fv_it.is_valid() && k < 3; ++fv_it, ++k)
				{
					...
					fidx[k] = (int)all_new_points.size();
					all_new_points.push_back(vp);
					all_new_ids.push_back(pool.GetID(vp));
				}
		...
		// Triangulate a GROUP of coplanar, connected cut faces in ONE CDT. Faces in
	// the group share interior mesh edges (e.g. the plate-top quad diagonal);
	// those interior edges are NOT used as boundary constraints, so the CDT is
	// free to triangulate across them consistently. Only the group's OUTER
	// boundary edges and the cut-curve constraints are inserted. This removes
	// the seam tearing caused by triangulating each face independently.
		void SplitGroupShared(
			const std::vector<int>& group,
			const std::map<int, std::vector<int>>& facePointIds,
			const std::map<int, std::vector<std::pair<int, int>>>& faceConstraints,
			const std::map<int, std::vector<int>>& edge_points,
			CanonicalPool& pool,
			float merge_eps_sq, float close_eps,
			...
			std::vector<Eigen::Vector3i>& all_new_indices)
		{
			...
			// group plane basis from the first valid face
			Eigen::Vector3f v0, v1, v2, e_normal, axis_u, axis_v, origin;
			...
			for (int fi : group)
			{
				...
				if (!fh.is_valid() || status(fh).deleted()) continue;
				GetFaceVertices(fh, v0, v1, v2);
				e_normal = (v1 - v0).cross(v2 - v0);
				if (e_normal.squaredNorm() < 1e-12f) continue;
				e_normal.normalize();
				axis_u = (v1 - v0).normalized();
				axis_v = e_normal.cross(axis_u).normalized();
				if (axis_u.cross(axis_v).dot(e_normal) < 0.0f) axis_v = -axis_v;
				origin = v0;
				basis_set = true;
				break;
			...
			std::vector<CDT::V2d<float>> cdt_points;
			std::map<int, CDT::VertInd> id_to_cdt;
			std::map<CDT::VertInd, int> cdt_to_id;
			...
			// register the group's corner vertices and remember winding reference
			bool is_base_ccw = false;
			{
				auto ccw = [](const Eigen::Vector2f& p1, const Eigen::Vector2f& p2, const Eigen::Vector2f& p3) -> float
					{ return (p2.x() - p1.x()) * (p3.y() - p1.y()) - (p2.y() - p1.y()) * (p3.x() - p1.x()); };
				Eigen::Vector2f u0 = to_uv(v0), u1 = to_uv(v1), u2 = to_uv(v2);
				is_base_ccw = ccw(u0, u1, u2) > 0.0f;
			}

			// collect the group's OUTER boundary halfedges (edge whose opposite
			// face is NOT in the group). Those become boundary constraint chains.
			std::vector<std::pair<CDT::VertInd, CDT::VertInd>> boundary_edges;

			auto add_vert_pos = [&](const Eigen::Vector3f& p) -> CDT::VertInd
				{
					int cid = pool.GetID(p);
					return add_canonical(cid);
				};
			...
			for (int fi : group)
			{
				...
				for (auto fh_it = cfh_iter(fh); fh_it.is_valid(); ++fh_it)
				{
					// is this an OUTER boundary halfedge of the group?
					auto opp = opposite_halfedge_handle(*fh_it);
					bool outer = true;
					if (!is_boundary(opp))
					{
						auto nf = face_handle(opp);
						if (nf.is_valid() && !status(nf).deleted() && group_set.count(nf.idx()))
							outer = false;   // shared interior edge -> skip
					}
					if (!outer) continue;
					...
					// build chain along this boundary edge using shared edge_points
					std::vector<std::pair<float, CDT::VertInd>> chain;
					chain.push_back({ 0.0f, add_vert_pos(a) });
					chain.push_back({ (b - a).squaredNorm(), add_vert_pos(b) });
			...
			// interior cut points (become CDT vertices)
			for (int fi : group)
			{
				auto fp_it = facePointIds.find(fi);
				if (fp_it != facePointIds.end())
					for (int cid : fp_it->second) add_canonical(cid);
			}

			// cut constraints from all faces in the group, linked into rings
			std::vector<std::pair<CDT::VertInd, CDT::VertInd>> internal_edges;
			for (int fi : group)
			{
				auto fc_it = faceConstraints.find(fi);
				if (fc_it == faceConstraints.end()) continue;
			...
			try
			{
				...
				// delete all original group faces
				for (int fi : group)
				{
					OpenMesh::FaceHandle fh = face_handle(fi);
					if (fh.is_valid() && !status(fh).deleted()) delete_face(fh, false);
				}

				auto uv_to_3d = [&](float ux, float uy) -> Eigen::Vector3f
					{
						// planar: reconstruct via origin + basis
						return origin + axis_u * ux + axis_v * uy;
					};
				...
				for (const auto& tri : cdt.triangles)
				{
					...
					for (int k = 0; k < 3; ++k)
					{
						...
						if (it != cdt_to_id.end()) { p[k] = pool.Point(it->second); pid[k] = it->second; }
						else { p[k] = uv_to_3d(cdt.vertices[vi[k]].x, cdt.vertices[vi[k]].y); pid[k] = local_id_counter--; }
					}

					int i0 = (int)all_new_points.size(); all_new_points.push_back(p[0]); all_new_ids.push_back(pid[0]);
					int i1 = (int)all_new_points.size(); all_new_points.push_back(p[1]); all_new_ids.push_back(pid[1]);
					int i2 = (int)all_new_points.size(); all_new_points.push_back(p[2]); all_new_ids.push_back(pid[2]);
					Eigen::Vector3i f_idx(i0, i1, i2);
					if (f_idx[0] == f_idx[1] || f_idx[1] == f_idx[2] || f_idx[2] == f_idx[0]) continue;
			...
			{
				std::cout << "[ERROR] CDT group: " << e.what() << std::endl;
			}
		}
		...
		// Close small boundary loops. Each patch triangle is oriented to agree
		// with the existing face across the loop edge it touches, so fills never
		// come out reversed. No centroid is created, so patches never bulge into
		// a pocket: the fill stays within the loop's own footprint (ear clip).
		int FillSmallBoundaryHoles(size_t max_loop_size = 8)
		{
			...
			auto pt = [&](OpenMesh::VertexHandle v) { return Eigen::Vector3f(point(v).data()); };

			// Add a triangle (a,b,c) but first flip its winding if that makes it
			// agree with whatever existing face sits across one of its edges.
			// Returns true if a face was added.
			auto add_oriented = [&](OpenMesh::VertexHandle a,
				OpenMesh::VertexHandle b,
				OpenMesh::VertexHandle c) -> bool
				{
					// desired normal: match the existing neighbor across edge a-b
					// (or b-c, c-a). A boundary halfedge h has its existing face
					// on the opposite side; our new face must point the same way.
					Eigen::Vector3f want(0, 0, 0);
					OpenMesh::VertexHandle pairs[3][2] = { {a,b},{b,c},{c,a} };
					for (auto& pr : pairs)
					{
						auto h = find_halfedge(pr[0], pr[1]);
						if (!h.is_valid()) continue;
						auto opp = opposite_halfedge_handle(h);
						auto ef = face_handle(opp);
						if (!ef.is_valid() || status(ef).deleted()) continue;
						Eigen::Vector3f v0, v1, v2;
						GetFaceVertices(ef, v0, v1, v2);
						want = (v1 - v0).cross(v2 - v0);
						break;
					}

					Eigen::Vector3f n = (pt(b) - pt(a)).cross(pt(c) - pt(a));
					OpenMesh::FaceHandle fh;
					if (want.squaredNorm() > 1e-20f && n.dot(want) < 0.0f)
						fh = add_face(a, c, b);   // flip to agree with neighbor
					else
						fh = add_face(a, b, c);
					if (!fh.is_valid())
					{
						// topological failure: try the other winding as a last resort
						fh = add_face(a, c, b);
					}
					return fh.is_valid();
				};
			...
			for (auto& loop : loops)
			{
				...
				if (loop.size() == 3)
				{
					if (add_oriented(loop[0], loop[1], loop[2])) filled++;
					continue;
				}

				// ear clip using only the loop's vertices (no new centroid)
				std::vector<OpenMesh::VertexHandle> poly = loop;

				// loop normal via Newell, used only for the convexity test
				Eigen::Vector3f nrm = Eigen::Vector3f::Zero();
				for (size_t i = 0; i < poly.size(); ++i)
				{
					Eigen::Vector3f p0 = pt(poly[i]);
					Eigen::Vector3f p1 = pt(poly[(i + 1) % poly.size()]);
					nrm.x() += (p0.y() - p1.y()) * (p0.z() + p1.z());
					nrm.y() += (p0.z() - p1.z()) * (p0.x() + p1.x());
					nrm.z() += (p0.x() - p1.x()) * (p0.y() + p1.y());
				}
				if (nrm.squaredNorm() < 1e-20f) continue;
				nrm.normalize();

				int added = 0, guard = 0;
				while (poly.size() > 3 && guard++ < 1000)
				{
					bool ear = false;
					int m = (int)poly.size();
					for (int i = 0; i < m; ++i)
					{
						OpenMesh::VertexHandle va = poly[(i + m - 1) % m];
						OpenMesh::VertexHandle vb = poly[i];
						OpenMesh::VertexHandle vc = poly[(i + 1) % m];
						Eigen::Vector3f p0 = pt(va), p1 = pt(vb), p2 = pt(vc);

						// convex corner w.r.t. loop normal
						if ((p1 - p0).cross(p2 - p1).dot(nrm) <= 0.0f) continue;

						// no other loop vertex inside this candidate ear
						bool clean = true;
						for (int j = 0; j < m; ++j)
						{
							if (j == (i + m - 1) % m || j == i || j == (i + 1) % m) continue;
							if (IsPointInTriangle(pt(poly[j]), p0, p1, p2)) { clean = false; break; }
						}
						if (!clean) continue;

						if (add_oriented(va, vb, vc))
						{
							poly.erase(poly.begin() + i);
							added++; ear = true; break;
						}
					}
					if (!ear) break;   // numeric stall; leave the remainder
				}
				if (poly.size() == 3)
				{
					if (add_oriented(poly[0], poly[1], poly[2])) added++;
				}
				if (added > 0) filled++;
			}
		...
		// Collect faces selected by a per-face boolean mask (true = keep).
		// If flip, reverse winding (subtracted volume).
		void CollectFacesByMask(
			const std::vector<char>& keep_face,
			bool flip,
			std::vector<Eigen::Vector3f>& out_points,
			std::vector<Eigen::Vector3i>& out_indices) const
		{
			...
			{
				if (status(*f_it).deleted()) continue;
				if (!keep_face[f_it->idx()]) continue;
