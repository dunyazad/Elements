...
void GUISystem::RenderRectangle(float x, float y, float width, float height, const Eigen::Vector4f& color)
{
    ...
    float vertices[6][4] = { { x, y, 0.0f, 0.0f }, { x + width, y, 1.0f, 0.0f }, { x + width, y + height, 1.0f, 1.0f }, { x, y, 0.0f, 0.0f }, { x + width, y + height, 1.0f, 1.0f }, { x, y + height, 0.0f, 1.0f } };
    glBindVertexArray(VAO); glBindBuffer(GL_ARRAY_BUFFER, VBO); glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices);
...
void GUISystem::RenderCircle(float x, float y, float radius, const Eigen::Vector4f& color)
{
    ...
    float vertices[6][4] = { { sx, sy, 0.0f, 0.0f }, { sx + d, sy, 1.0f, 0.0f }, { sx + d, sy + d, 1.0f, 1.0f }, { sx, sy, 0.0f, 0.0f }, { sx + d, sy + d, 1.0f, 1.0f }, { sx, sy + d, 0.0f, 1.0f } };
    glBindVertexArray(VAO); glBindBuffer(GL_ARRAY_BUFFER, VBO); glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices);
    glDrawArrays(GL_TRIANGLES, 0, 6);
    glBindBuffer(GL_ARRAY_BUFFER, 0); glBindVertexArray(0); glEnable(GL_DEPTH_TEST); glEnable(GL_CULL_FACE);
}
...
void GUISystem::RenderText(const std::string& text, float x, float y, float textWidth, float targetFontSize, const Eigen::Vector4f& color, TextHAlign hAlign, TextVAlign vAlign)
{
    ...
    for (uint32_t cp : codepoints) {
        ...
        float vertices[6][4] = { { x0, y0, q.s0, q.t0 }, { x1, y0, q.s1, q.t0 }, { x1, y1, q.s1, q.t1 }, { x0, y0, q.s0, q.t0 }, { x1, y1, q.s1, q.t1 }, { x0, y1, q.s0, q.t1 } };
