...
#include <Helium/Systems/EventSystem.h> // EventSystem 헤더 추가
...
{
    // 직접 Dispatcher를 가져오지 않고 EventSystem을 통해 Trigger 함
    auto eventSystem = Helium.GetEventSystem();
    if (!eventSystem) return;

    int currentMods = 0;

    if (GetKeyState(VK_LSHIFT) & 0x8000)   currentMods |= KeyModifiers::LeftShift | KeyModifiers::Shift;
    if (GetKeyState(VK_RSHIFT) & 0x8000)   currentMods |= KeyModifiers::RightShift | KeyModifiers::Shift;
    if (GetKeyState(VK_LCONTROL) & 0x8000) currentMods |= KeyModifiers::LeftControl | KeyModifiers::Control;
    if (GetKeyState(VK_RCONTROL) & 0x8000) currentMods |= KeyModifiers::RightControl | KeyModifiers::Control;
    if (GetKeyState(VK_LMENU) & 0x8000)    currentMods |= KeyModifiers::LeftAlt | KeyModifiers::Alt;
    if (GetKeyState(VK_RMENU) & 0x8000)    currentMods |= KeyModifiers::RightAlt | KeyModifiers::Alt;
    ...
    case WM_SYSKEYDOWN:
    {
        ...
        // 키 상태 추적을 위해 int로 변환하여 인덱싱 (필요한 경우)
        int keyInt = static_cast<int>(key);
        if (keyInt < 256)
        {
            keyStates[keyInt] = true;
            // EventSystem을 통해 이벤트 전파 (레이어 순서대로 처리됨)
            eventSystem->Trigger<KeyEvent>(key, 1, currentMods);
        }
        break;
    }
    ...
    case WM_SYSKEYUP:
    {
        ...
        if (keyInt < 256)
        {
            keyStates[keyInt] = false;
            eventSystem->Trigger<KeyEvent>(key, 0, currentMods);
        }
        break;
    }
    ...
    case WM_MOUSEMOVE:
    {
        ...
        eventSystem->Trigger<MousePositionEvent>(mousePos.x(), mousePos.y());
        break;
    }
...
void InputSystem::OnMouseWheel(float xoffset, float yoffset)
{
    ...
    if (eventSystem)
    {
        eventSystem->Trigger<MouseWheelEvent>(xoffset, yoffset);
    }
}
