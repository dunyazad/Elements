...
namespace PointProcessing
{
	...
	std::vector<uint8_t> ROR::Process(const PointProcessorParameters& parameters)
	{
		int pointCloudID = -1;
		float radius = 0.3f;
				...
				for (size_t i = 0; i < displayCount; ++i)
				{
					...
					else
					{
						...
						else if (visualizationMode == (int)PointVisualizationMode::Gradient)
						{
							...
							colorRGBA = Color::GetHeatMapColor((float)count, (float)minNeighborsInRadius, maxSafeCount, 1.0f);
						}
					}
